...
const steps = ['Factura origen', 'Tipo de nota', 'Líneas o alcance', 'Motivo', 'Confirmación'];
...
export default function CrearNotaCreditoPage() {
  ...
  const [concepto, setConcepto] = useState('DEVOLUCION_PARCIAL');
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        showNotification({ message: 'No fue posible cargar facturas origen.', type: 'error' });
      } finally {
  ...
  const loadVentaLines = async (factura: FacturaOption) => {
    if (!factura.ventaId) {
      showNotification({ message: 'La factura seleccionada no tiene venta asociada para cargar líneas.', type: 'error' });
      return;
    }
    setLoading(true);
    try {
      ...
      notasExistentes.forEach((nota) => {
        const aplica = nota.factura_asociada === factura.numero && ['ACEPTADA', 'ENVIADA_A_FACTUS'].includes((nota.estado_local || '').toUpperCase());
        if (!aplica) return;
        (nota.detalles || []).forEach((linea) => {
          acumulado.set(linea.detalle_venta_original, (acumulado.get(linea.detalle_venta_original) || 0) + toNumber(linea.cantidad_a_acreditar));
        });
      });
  ...
  const handleEmitir = async () => {
    ...
    try {
      ...
      showNotification({ message: 'No fue posible emitir la nota crédito.', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-4 px-6 py-6">
      <div className="rounded-xl bg-white p-5 shadow">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-blue-600">Facturación / Notas crédito / Crear</p>
            <h2 className="text-3xl font-semibold text-slate-900">Crear nota crédito</h2>
            <p className="text-sm text-slate-500">Flujo guiado para reversar parcialmente o totalmente una factura de forma segura.</p>
          </div>
          <Link to="/listados/notas-credito" className="rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">
            Volver al listado
          </Link>
        </div>
        ...
        <ol className="mt-4 grid gap-2 text-xs sm:grid-cols-5">
          {steps.map((step, index) => (
            <li key={step} className={`rounded-md border px-3 py-2 ${index <= currentStep ? 'border-blue-300 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-500'}`}>
              {index + 1}. {step}
            </li>
          ))}
        </ol>
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_320px]">
        <div className="space-y-4">
          <section className="rounded-xl bg-white p-5 shadow">
            ...
            <p className="mb-3 text-sm text-slate-500">Busca por número o cliente y elige la factura que deseas corregir.</p>
            <input
              ...
              {loadingFacturas ? (
                ...
                facturasFiltradas.map((factura) => (
                  <button
                    ...
                    className={`flex w-full items-center justify-between border-b border-slate-100 px-3 py-2 text-left text-sm hover:bg-slate-50 ${facturaSeleccionadaId === factura.id ? 'bg-blue-50' : ''}`}
                  >
                    <span>
            ...
            {facturaSeleccionada && (
              <div className="mt-4 rounded-lg border border-blue-100 bg-blue-50 p-4">
                <p className="text-sm font-semibold text-blue-900">Factura seleccionada: {facturaSeleccionada.numero}</p>
                <div className="mt-2 grid gap-2 text-sm text-slate-700 sm:grid-cols-2">
                  ...
                  <p><strong>Total:</strong> {money.format(facturaSeleccionada.total)}</p>
                  <p><strong>CUFE:</strong> {facturaSeleccionada.cufe || 'No disponible'}</p>
                  <p className="sm:col-span-2"><strong>Estado electrónico:</strong> <EstadoNotaCreditoBadge estado={facturaSeleccionada.estado} /></p>
                  <p className="sm:col-span-2"><strong>Notas crédito previas:</strong> {notasPreviasFactura.length}</p>
                </div>
              </div>
            )}
          </section>
          ...
          <section className="rounded-xl bg-white p-5 shadow">
            <h3 className="text-lg font-semibold text-slate-800">2) Tipo de nota crédito</h3>
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <button
                type="button"
                onClick={() => setTipoNota('PARCIAL')}
                className={`rounded-lg border p-4 text-left ${tipoNota === 'PARCIAL' ? 'border-blue-500 bg-blue-50' : 'border-slate-200'}`}
              >
                <p className="font-semibold text-slate-900">Nota crédito parcial</p>
                <p className="text-sm text-slate-600">Úsala para reversar uno o varios productos, con control por cantidad y stock.</p>
              </button>
              <button
                type="button"
                onClick={() => setTipoNota('TOTAL')}
                className={`rounded-lg border p-4 text-left ${tipoNota === 'TOTAL' ? 'border-blue-500 bg-blue-50' : 'border-slate-200'}`}
              >
                <p className="font-semibold text-slate-900">Nota crédito total</p>
                <p className="text-sm text-slate-600">Revierte toda la factura en una sola operación guiada y confirmada.</p>
              </button>
            </div>
          </section>

          <section className="rounded-xl bg-white p-5 shadow">
            <h3 className="text-lg font-semibold text-slate-800">3) {tipoNota === 'TOTAL' ? 'Alcance total' : 'Selección de productos'}</h3>
            {tipoNota === 'TOTAL' ? (
              <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
                Esta acción acreditará todas las líneas disponibles de la factura. Solo debes confirmar motivo e inventario.
              </div>
            ) : (
              <div className="mt-3 overflow-auto rounded-md border border-slate-200">
                <table className="min-w-[980px] divide-y divide-slate-200 text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {lineas.map((linea) => (
                      <tr key={linea.detalleId} className={linea.disponible <= 0 ? 'bg-slate-50 text-slate-400' : ''}>
                        <td className="px-3 py-2">
                          <input
                            ...
                            onChange={(event) =>
                              setLineas((prev) => prev.map((item) => (item.detalleId === linea.detalleId ? { ...item, selected: event.target.checked } : item)))
                            }
                          />
                        </td>
                        ...
                        <td className="px-3 py-2">
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              setLineas((prev) =>
                                prev.map((item) => (item.detalleId === linea.detalleId ? { ...item, cantidadAcreditar: nuevaCantidad } : item)),
                              );
                            }}
                        ...
                        <td className="px-3 py-2">
                          <input
                            ...
                            onChange={(event) =>
                              setLineas((prev) =>
                                prev.map((item) => (item.detalleId === linea.detalleId ? { ...item, afectaInventario: event.target.checked } : item)),
                              )
                            }
                          />
                        </td>
          ...
          <section className="rounded-xl bg-white p-5 shadow">
            <h3 className="text-lg font-semibold text-slate-800">4) Motivo y observaciones</h3>
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <label className="text-sm text-slate-700 md:col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Ejemplo: devolución por error en producto entregado"
                  className="mt-1 min-h-24 w-full rounded-md border border-slate-300 px-3 py-2"
                />
              </label>
              <label className="text-sm text-slate-700">
                Concepto de corrección
                <select value={concepto} onChange={(event) => setConcepto(event.target.value)} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2">
                  <option value="DEVOLUCION_PARCIAL">Devolución parcial</option>
                  <option value="ANULACION_TOTAL">Anulación total</option>
                  <option value="AJUSTE_TRIBUTARIO">Ajuste tributario</option>
                </select>
              </label>
              <label className="text-sm text-slate-700">
                Observaciones
                <input value={observaciones} onChange={(event) => setObservaciones(event.target.value)} placeholder="Opcional" className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
              </label>
            </div>
          </section>

          <section className="rounded-xl bg-white p-5 shadow">
            <h3 className="text-lg font-semibold text-slate-800">5) Confirmación</h3>
            <p className="mt-2 text-sm text-slate-600">Verifica resumen final antes de emitir a Factus.</p>
            <div className="mt-4 flex flex-wrap gap-2">
              <button type="button" onClick={handleEmitir} disabled={loading} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60">
                {loading ? 'Emitiendo...' : 'Emitir nota crédito'}
              </button>
              <button type="button" className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700" onClick={() => showNotification({ message: 'Borrador local pendiente de implementar en backend.', type: 'info' })}>
                Guardar borrador
              </button>
            </div>
          </section>
        ...
        <aside className="h-fit rounded-xl bg-white p-5 shadow xl:sticky xl:top-4">
          ...
          <div className="mt-3 space-y-2 text-sm text-slate-700">
            <p><strong>Factura origen:</strong> {facturaSeleccionada?.numero || 'Sin seleccionar'}</p>
            <p><strong>Tipo:</strong> {tipoNota === 'TOTAL' ? 'Nota crédito total' : 'Nota crédito parcial'}</p>
            <p><strong>Subtotal acreditado:</strong> {money.format(resumen.subtotal)}</p>
            <p><strong>Impuestos acreditados:</strong> {money.format(resumen.impuestos)}</p>
            <p><strong>Total nota crédito:</strong> {money.format(resumen.total)}</p>
            <p><strong>Líneas afectadas:</strong> {resumen.lineas}</p>
            <p><strong>Unidades:</strong> {resumen.unidades}</p>
            <p><strong>Devuelve stock:</strong> {resumen.devuelveStock ? 'Sí' : 'No'}</p>
            <p><strong>Estado esperado factura:</strong> {tipoNota === 'TOTAL' ? 'CREDITADA_TOTAL' : 'CREDITADA_PARCIAL'}</p>
          </div>
