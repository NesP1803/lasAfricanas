    ...
    <div className="mt-6 rounded-xl border border-slate-200 bg-white p-4">
      ...
      {(activeTab === 'resumen' || activeTab === 'consecutivos') && (
        <div className="mt-4 rounded-lg border border-slate-200 p-3">
          ...
          <button type="button" onClick={onSaveFacturacion} className="mt-3 inline-flex items-center gap-2 rounded bg-blue-600 px-3 py-2 text-xs font-semibold text-white"><Save size={14} /> Guardar numeración local</button>
        </div>
      )}

      {activeTab === 'rangos' && (
        <div className="mt-4 rounded-lg border border-slate-200 p-3">
          <h3 className="mb-4 text-4 font-semibold text-slate-700">Rangos de numeración</h3>
          <div className="mb-3 flex flex-wrap items-end gap-3">
            ...
            <button type="button" className="ml-auto inline-flex items-center gap-2 rounded bg-blue-600 px-3 py-2 text-xs font-semibold text-white" onClick={() => setShowModal(true)} disabled={!isAdmin}><Plus size={14} /> Nuevo rango de numeración</button>
          </div>
          ...
          <div className="overflow-auto rounded border border-slate-200">
            <table className="min-w-full text-xs">
              ...
              <tbody>
                {rangos.map((rango) => (
                  <tr key={rango.id} className={`border-t border-slate-100 ${rango.is_expired_remote ? 'bg-red-100/70' : ''}`}>
                    ...
                    <td className="px-2 py-2"><div className="flex flex-wrap gap-1">
                      <button className="rounded bg-slate-100 px-2 py-1 text-blue-600" title="Ver" onClick={async () => setDetalleRango(await configuracionAPI.obtenerDetalleRangoFacturacion(rango.id))}><Eye size={14} /></button>
                      <button className="rounded bg-slate-100 px-2 py-1 text-blue-600" title="Seleccionar" onClick={() => configuracionAPI.seleccionarActivoRangoFacturacion(rango.id, rango.document_code).then(loadData)} disabled={!isAdmin}>✓</button>
                      <button className={`rounded px-2 py-1 ${rango.is_active_remote ? 'bg-blue-600 text-white' : 'bg-slate-300 text-slate-700'}`} title="Activar/Desactivar" onClick={() => toggleActivo(rango)} disabled={!isAdmin}>{rango.is_active_remote ? 'On' : 'Off'}</button>
                      <button className="rounded bg-red-100 px-2 py-1 text-red-700" title="Eliminar" onClick={() => configuracionAPI.eliminarRangoFacturacion(rango.id).then(loadData)} disabled={!isAdmin}><Trash2 size={14} /></button>
                    </div></td>
                  </tr>
                ))}
              </tbody>
      ...
      {activeTab === 'remisiones' && (
        <div className="mt-4 rounded-lg border border-slate-200 p-3">
          ...
          <button className="mt-3 inline-flex items-center gap-2 rounded bg-blue-600 px-3 py-2 text-xs font-semibold text-white" onClick={() => configuracionAPI.actualizarNumeracionRemision({ prefix: remision.prefix || 'REM', current: remision.current || 1, range_from: remision.range_from || 1, range_to: remision.range_to || 99999999, resolution_reference: remision.resolution_reference || '', notes: remision.notes || '' }).then(loadData)} disabled={!isAdmin}><Save size={14} /> Guardar</button>
        </div>
      )}

      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-3">
          <div className="w-full max-w-6xl rounded-lg bg-[#f2f4f8] p-4">
            <h3 className="text-2xl font-semibold text-slate-700">Nuevo rango de numeración</h3>
            <div className="mt-3 grid grid-cols-2 gap-2">
              <button className={`rounded px-3 py-2 text-sm font-semibold ${createTab === 'autorizado' ? 'bg-blue-700 text-white' : 'bg-blue-400 text-white'}`} onClick={() => setCreateTab('autorizado')}>Rango autorizado</button>
              <button className={`rounded px-3 py-2 text-sm font-semibold ${createTab === 'manual' ? 'bg-blue-700 text-white' : 'bg-blue-400 text-white'}`} onClick={() => setCreateTab('manual')}>Rango manual</button>
            </div>
            <div className="mt-4 rounded-lg border border-slate-200 bg-white p-3">
              ...
              {createTab === 'autorizado' ? <div className="flex items-end"><button className="rounded bg-blue-600 px-5 py-2 text-sm font-semibold text-white" onClick={handleBuscarRangosAutorizados}>Buscar</button></div> : null}
              ...
              <div className="mt-3 rounded-lg border bg-white p-4">
                ...
                <div className="max-h-40 overflow-auto rounded border p-2">
                  {availableRanges.map((item) => (
                    <button key={String(item.id || item.numbering_range_id)} className="block w-full rounded px-2 py-1 text-center text-2xl font-semibold text-slate-700 hover:bg-slate-100" onClick={() => applyRemoteRange(item)}>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button className="rounded bg-blue-600 px-3 py-2 text-sm font-semibold text-white" onClick={handleCreate}>Registrar</button>
            </div>
          </div>
        </div>
      )}
