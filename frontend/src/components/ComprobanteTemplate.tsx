...
const calcularResumenIva = (detalles: DocumentoDetalle[]) => {
  const resumen = new Map<number, { base: number; iva: number; total: number }>();
  detalles.forEach((detalle) => {
    const base = detalle.cantidad * detalle.precioUnitario - detalle.descuento;
    const iva = base * (detalle.ivaPorcentaje / 100);
    const total = base + iva;
    const acumulado = resumen.get(detalle.ivaPorcentaje) || { base: 0, iva: 0, total: 0 };
    resumen.set(detalle.ivaPorcentaje, {
      base: acumulado.base + base,
      iva: acumulado.iva + iva,
      total: acumulado.total + total,
    });
  });
  return Array.from(resumen.entries()).map(([porcentaje, valores]) => ({
    porcentaje,
    ...valores,
  }));
};
...
export default function ComprobanteTemplate({
  ...
  const resumenIva = calcularResumenIva(detallesMostrar);
    ...
    return (
      <div className="mx-auto w-full max-w-[340px] border border-slate-300 bg-white p-4 text-[10px] text-slate-800">
        <div className="text-center">
          ...
          {resolucion ? (
            <p className="mt-1 text-[9px] text-slate-500">{resolucion}</p>
          ) : null}
        </div>
