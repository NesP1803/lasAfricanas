...
export const auditoriaApi = {
  listar: async (params?: {
    ...
    const response = await apiClient.get('/config/auditoria/', { params });
    return response.data;
  },
};

// API de Configuración de Facturación
export const configuracionFacturacionApi = {
  obtener: async (): Promise<ConfiguracionFacturacion> => {
    const response = await apiClient.get('/config/facturacion/');
    return response.data;
  },

  actualizar: async (id: number, data: Partial<ConfiguracionFacturacion>): Promise<ConfiguracionFacturacion> => {
    const response = await apiClient.patch(`/config/facturacion/${id}/`, data);
    return response.data;
  },
};
...
export const usuariosApi = {
  listar: async (incluirInactivos = false): Promise<Usuario[]> => {
  ...
  activar: async (id: number): Promise<void> => {
    await apiClient.post(`/config/usuarios/${id}/activar/`);
  },
