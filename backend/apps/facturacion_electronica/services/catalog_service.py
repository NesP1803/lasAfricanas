...
from apps.facturacion_electronica.services.factus_client import FactusAPIError, FactusClient
...
class CatalogSyncService:
    ...
    def _fetch_catalog(self, endpoint: str) -> list[dict]:
        ...
        try:
            ...
            raise FactusAPIError(f'Error consultando catálogo Factus {endpoint}: {detail}') from exc
        except error.URLError as exc:
            raise FactusAPIError(
                f'Error de red consultando catálogo Factus {endpoint}: {exc.reason}'
            ) from exc
    ...
    def _sync_catalog(self, model, endpoint: str) -> dict:
        ...
        current_ids = [item['factus_id'] for item in normalized_items]
    ...
    def sync_identification_documents(self) -> dict:
        return self._sync_catalog(
            DocumentoIdentificacionFactus,
            self.endpoints['identification_documents'],
        )
